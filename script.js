document.addEventListener('DOMContentLoaded', () => {
    ...
    const chatInputArea = document.querySelector('.chat-input-area'); // <<--- 이 줄 확인 및 수정
    // 모달 내부 요소들 (필요시 추가)
    ...
    const ALL_TAROT_CARD_IDS = [ /* 제공해주신 배열 그대로 */
    ...
    const syncTypeData = {
        overview: {
            image: "img/sync_type/overview_default.png", // 총평 기본 이미지
            text: "당신의 싱크타입에 대한 전반적인 분석입니다. 당신은 다면적인 성향을 가지고 있으며, 상황에 따라 유연하게 대처하는 능력이 돋보입니다. 때로는 내면의 목소리에 더 귀 기울일 필요가 있습니다."
        },
        nebula: {
            image: "img/sync_type/nebula_default.png", // 성운 기본 이미지 (사용자별 이미지로 교체 필요)
            text: "당신은 [사용자소속성운] 성운에 속해 있습니다. 이 성운의 특징은 [성운 특징]이며, 당신이 이 성운에 속한 이유는 [사용자가성운에속한이유]입니다. 당신의 잠재력을 발휘하여 성운의 빛을 더욱 밝혀주세요."
        },
        syncTypeDetail: {
            image: "img/sync_type/type_default.png", // 싱크타입별 기본 이미지 (사용자 싱크타입 이미지로 교체 필요)
            text: "당신의 결정된 싱크타입은 [결정된싱크타입] ([맞춤싱크타입이름])입니다. 이 타입은 [싱크타입 설명] 특징을 가지며, 당신의 강점은 [강점], 보완할 점은 [보완점]입니다."
        }
    };
    ...
    function initializeUserProfile() {
        ...
        let defaultProfile = {
            "사용자이름": "테스트유저", "사용자애칭": "테스터", "사용자가좋아하는것": "코딩", "사용자의마음을아프게하는것": "버그",
            "사용자가싫어하는것": "미팅", "사용자의나이성별": "30대 비공개", "사용자의고민": "오늘 저녁 뭐먹지",
            ...
            "결정된싱크타입": "몽상가", "사용자소속성운": "오리온", "사용자가성운에속한이유": "창의적 사고",
            "맞춤싱크타입이름": "별을 쫓는 몽상가", "사용자의감정상태": "평온함",
            "선택된타로카드들": [], 
            "지금까지수집된타로카드": ["major_00_fool_upright", "wands_01_ace_upright", "cups_02_two_reversed"], // 테스트용 수집 카드
            "시나리오": null, "메뉴단계": 1,
            "싱크타입단계": "결정됨" // 테스트용
        };
    ...
    const menuConfigurations = { /* 이전과 동일 */
    ...
    function updateTarotCollectionModal() {
        ...
        if (!userProfile || !tarotCollectedCountEl || !tarotTotalCountEl || !tarotGaugeFillEl || !tarotCardGridEl) {
            console.error("[Modal] 타로 콜렉션 모달 필수 요소 없음.");
            return;
        }
        ...
        tarotCardGridEl.innerHTML = ''; // 기존 카드 비우기
        ALL_TAROT_CARD_IDS.forEach(cardId => {
            ...
            if (!isCollected) {
                cardItem.classList.add('not-collected'); // 수집 안된 카드 스타일 (CSS에서 정의)
            }

            const img = document.createElement('img');
            img.src = `img/tarot/${cardId}.png`;
            img.alt = cardId.replace(/_/g, ' '); // 카드 이름 표시 (예: major 00 fool upright)
            // img.loading = 'lazy'; // 이미지 지연 로딩 (선택적)
    ...
    document.addEventListener('click', (e) => { if (moreOptionsPanel.classList.contains('active') && !moreOptionsBtn.contains(e.target) && !moreOptionsPanel.contains(e.target)) { console.log("[Panel] 외부 클릭으로 패널 닫기."); moreOptionsPanel.classList.remove('active'); moreOptionsBtn.classList.remove('active'); } });
    let resizeTimeout;
